
def get_house_slots():
    """
    Définit les positions exactes des 250 maisons.
    Format : (x, y)
    """
    return [
        
        # --- QUARTIER BAS-GAUCHE (~80 maisons) ---
        (30, 750), (90, 750), (120, 750), (150, 750), (180, 750), (210, 750), (240, 750), (270, 750), (300, 750), (330, 750),
        (30, 720), (30, 690), (30, 660), (30, 630), (30, 600), (30, 570), (30, 540),
        (60, 540), (90, 540), (120, 540), (150, 540), (210, 540), (240, 540), (270, 540), (300, 540),
        (120, 570), (120, 600), (120, 630), (120, 690), (120, 720),
        (60, 630), (90, 630),
        (150, 630), (240, 630), (270, 630), (300, 630), (210, 630),
        (330, 630), (330, 600), (330, 570), 
        (300, 570), (270, 570), (240, 570), 
        (240, 600), 
        (180, 690), (210, 690), (240, 690), (270, 690),
        (270, 720), (240, 720), (210, 720), (180, 720),
        (60, 600), (90, 600),
        (150, 690),
        (300, 690), (300, 720),
        (330, 690), (330, 720),
        (30, 510), (60, 510), (90, 510), (30, 480), (30, 450), (30, 420), (30, 390), (120, 510), (150, 510),
        (360, 750), (360, 720), (360, 690), (360, 630),      


        # --- QUARTIER EST (Étendu vers le bas) ---
        (950, 310), (950, 340), (950, 370), (950, 400), (950, 430), (950, 460), (950, 490), (950, 520), (950, 550),
        (950, 580), (950, 610),
        (860, 370), (890, 370), (920, 370), 
        (980, 370), (1010, 370), (1040, 370), (1070, 370),
        (1040, 400), (1040, 430), (1040, 460), 
        (1070, 460), (1100, 460),
        (1100, 430), (1100, 400), 
        (1070, 400),
        (920, 460), (890, 460), (860, 460), 
        (860, 520),
        (890, 520), (920, 520),
        (890, 400),
        (920, 550), (980, 550), (1010, 550), (1040, 550),
        (860, 550), (890, 550),
        (860, 580), (890, 580), (920, 580),
        (890, 610), (920, 610),
        (980, 580), (1010, 580), (1040, 580),
        (980, 610), (1010, 610), (1040, 610),
        (950, 280), 
        (980, 280), (1010, 280), (980, 310),
        (1010, 250), (1010, 310),
        (1130, 350), (1130, 380), (1130, 410), (1130, 440), (1130, 470), (1130, 320),
        (1130, 500), (1130, 530), (1130, 560),
        (980, 400), (980, 430), (980, 460), (980, 490), (980, 520),
        (1010, 400), (1010, 430), (1010, 460), (1010, 490), (1010, 520),
        (920, 490), (980, 340), 


        # --- QUARTIER NORD (Centré sur 550, 100) ---
        (340, 70), (370, 70), (400, 70), (430, 70), (460, 70), (490, 70), (520, 70),
        (550, 70),
        (580, 70), (610, 70), (640, 70), (670, 70), (700, 70), (730, 70), (760, 70),
        (550, 100), (550, 130), (550, 160), (550, 190), (550, 220),
        (400, 100), (400, 130), (400, 160), (400, 190),
        (460, 190), (490, 190), (520, 190),
        (700, 100), (700, 130), (700, 160), (700, 190), 
        (640, 190), (610, 190), (580, 190),
        (460, 130), (460, 160),
        (640, 130), (640, 160),
        (460, 250), (490, 250), (520, 250), 
        (550, 250), 
        (580, 250), (610, 250), (640, 250), (670, 250),
        (520, 40), (550, 40), (580, 40),
        (400, 220), (370, 220), (370, 250),
        (730, 220), (760, 220), (760, 250),
        (490, 130), (610, 130),
        (520, 130), (580, 130),
        (610, 40), (640, 40), (670, 40), (700, 40), (730, 40), 

    ]

def get_vegetation_slots():
    """
    Définit les positions de la végétation.
    Returns: dict with 'trees_1', 'trees_2', 'bushes', 'flowers'
    """
    return {
        "trees_1": [
            (275, 110), (275, 135), (275, 160), (275, 185),
        ],
        "trees_2": [],
        "bushes": [],
        "flowers": []
    }

def get_road_network():
    """
    Renvoie une liste de 'chemins'.
    Chaque chemin est une liste de points (x, y) connectés.
    """
    return [

        [
            (415, 282), 
            (785, 282),
            (785, 573),
            (415, 573),
            (415, 282)
        ],

        [
            (596, 311),
            (596, 282)
        ],

        [
            (747, 431),
            (785, 431)
        ],

        [
            (583, 548),
            (583, 573)
        ],

        [
            (415, 432),
            (450, 432)
        ],

        [
            (415, 573),
            (415, 689),
            (490, 689)
        ],

        [
            (785, 573),
            (785, 690),
            (709, 690)
        ],

        [
            (415, 432),
            (232, 432),
            (232, 209)
        ],

        [
            (785, 282),
            (832, 282),
            (832, 166),
            (868, 166),
        ]

    ]

def get_simple_road_network():
    """
    Renvoie une liste de 'chemins' pour les routes simples (sans trottoir).
    """
    return [
        [
            (232, 390),
            (50, 390),
            (50, 485),
            (180, 485),
            (232, 485),
            (232, 432),
        ],
        [
            (180, 485),
            (180, 575),
            (180, 660),
            (300, 660),
            (415, 660),
            (415,770),
            (285,770),
            (165,770),
            (60,770),
            (60, 660),
            (180, 660),
        ],
        [
            (60,770),
            (25,770),
        ],
        [
            (832, 248),
            (923, 248),
            (987, 248)
        ],
        [
            (923, 248),
            (923, 340),
            (785, 340)
        ],
        [
            (785, 431),
            (920, 431)
        ],
        [
            (785, 490),
            (890, 490)
        ],
        [
            (785, 630),
            (880, 630),
            (980, 630),
            (1090, 630),
            (1090, 558),
            (1090, 488)
        ],
        [
            (430, 282),
            (430, 220),
            (430, 100),
            (520, 100)
        ],
        [
            (430, 220),
            (520, 220)
        ],
        [
            (705, 282),
            (705, 220),
            (670, 220),
            (670, 100),
            (580, 100)
        ],
        [
            (670, 220),
            (580, 220)
        ]
    ]
